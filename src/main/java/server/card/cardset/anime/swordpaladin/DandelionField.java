...
public class DandelionField extends AmuletText {
    ...
    private static final String LISTENER_DESCRIPTION = "Whenever an enemy minion comes into play, if it has 1 or less health, <b>Banish</b> it, otherwise deal 1 damage to it.";
    ...
    public static final TooltipAmulet TOOLTIP = new TooltipAmulet(NAME, DESCRIPTION, () -> new Animation("card/anime/dandelionfield.png"),
            ...
            new Vector2f(), -1,
            () -> List.of(Tooltip.COUNTDOWN, Tooltip.BANISH),
            List.of());
        ...
        return List.of(new Effect(DESCRIPTION, EffectStats.builder()
            ...
            public ResolverWithDescription onListenEventWhileInPlay(Event event) {
                if (event != null) {
                    ...
                    List<Minion> minionsToBanish = groups.get(true);
                    List<Minion> minionsToDamage = groups.get(false);
                    if (!minionsToBanish.isEmpty()) {
                        return new ResolverWithDescription(LISTENER_DESCRIPTION, new BanishResolver(minionsToBanish));
                    }
